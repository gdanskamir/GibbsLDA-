...
void utils::sort_topn(vector<pair<int, double> > & vect, int left, int right, int top_n) {
    pair<int, double> max_pivot;
    int max_iter = 0;

    int begin = left;
    while (begin < top_n && begin <= right) {
        max_pivot = vect[begin];
        
        int inner_begin = begin + 1;
        while (inner_begin <= right) {
            if (vect[inner_begin].second > max_pivot.second) {
                max_pivot = vect[inner_begin];
                max_iter = inner_begin;
            }
            inner_begin = inner_begin + 1;
        }

        if (max_iter != begin) {
            vect[max_iter] = vect[begin];
            vect[begin] = max_pivot;
        } 
        begin = begin + 1;
    }
}
...
void utils::get_current_timestr(char* time_str, size_t time_str_buf_len) {
    time_t t = time(0);
    time_str[0] = 0;
    strftime(time_str, time_str_buf_len, "%Y-%m-%d %H:%M:%S",localtime(&t));
}

void utils::timer_start(timespec *aStartTime) {
    clock_gettime(CLOCK_REALTIME, aStartTime);
}

long utils::timer_end(timespec *aStartTime) {
    timespec endTime;
