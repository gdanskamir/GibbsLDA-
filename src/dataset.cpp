...
int dataset::read_trndata(string dfile, string wordmapfile) {
    ...
    for (int i = 0; i < M; i++) {
        ...
        for (int j = 0; j < length; j++) {
            ...
            utils::timer_start(&ts);
            it = word2id.find(strtok.token(j));
            time_use = utils::timer_end(&ts);
            time_find = time_find + time_use;
            if (it == word2id.end()) {
                ...
                time_use = utils::timer_end(&ts);
                time_insert = time_insert + time_use;
            } else {
    ...
    utils::get_current_timestr(time_str, BUFF_SIZE_SHORT);
    fprintf(stderr, "[%s] time find [%ld] time insert [%ld]\n", time_str, time_find, time_insert);
    fclose(fin);
