...
int model::init(int argc, char ** argv) {
    ...
    fprintf(stderr, "[%s] begin to init\n", time_str);
...
int model::save_model(string model_name) {
    ...
    utils::get_current_timestr(time_str, BUFF_SIZE_SHORT);
    fprintf(stderr, "[%s] save model over\n", time_str);
...
int model::save_model_twords(string filename) {
    ...
    utils::sort_topn(words_probs, 0, words_probs.size() - 1, MODEL_TWORDS_TOPNO);
    //utils::quicksort(words_probs, 0, words_probs.size() - 1);
	fprintf(fout, "Topic %dth:\n", k);
    //for (int i = 0; i < twords; i++) {
    for (int i = 0; i < MODEL_TWORDS_TOPNO; i++) {
...
void model::estimate() {
    ...
    utils::get_current_timestr(time_str, BUFF_SIZE_SHORT);
    printf("[%s] Sampling %d iterations!\n", time_str, niters);
    int last_iter = liter;
	...
	if (savestep > 0) {
		...
		// saving the model
        utils::get_current_timestr(time_str, BUFF_SIZE_SHORT);
		printf("[%s] Saving the model at iteration %d ...\n", time_str, liter);
		compute_theta();
		compute_phi();
    ...
    utils::get_current_timestr(time_str, BUFF_SIZE_SHORT);
    printf("[%s] Gibbs sampling completed! Saving the final model!\n", time_str);
    compute_theta();
    compute_phi();
    liter--;
...
void model::inference() {
    ...
    utils::get_current_timestr(time_str, BUFF_SIZE_SHORT);
    printf("[%s] Sampling %d iterations for inference!\n", time_str, niters);
    ...
    utils::get_current_timestr(time_str, BUFF_SIZE_SHORT);
    printf("[%s] Gibbs sampling for inference completed! Saving the inference outputs!\n", time_str);
    compute_newtheta();
    compute_newphi();
    inf_liter--;
